-file("src/otmp_common_pb.erl", 1).

-module(otmp_common_pb).

-export([encode_pong/1, decode_pong/1,
	 delimited_decode_pong/1, encode_ping/1, decode_ping/1,
	 delimited_decode_ping/1]).
...
-record(pong, {id, sessionid}).

-record(ping, {id, sessionid}).
...
encode_pong(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pong(Record) when is_record(Record, pong) ->
    encode(pong, Record).

encode_ping(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_ping(Record) when is_record(Record, ping) ->
    encode(ping, Record).

encode(ping, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(ping, Record) ->
    [iolist(ping, Record) | encode_extensions(Record)];
encode(pong, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pong, Record) ->
    [iolist(pong, Record) | encode_extensions(Record)].
...
iolist(ping, Record) ->
    [pack(1, required, with_default(Record#ping.id, none),
	  int32, []),
     pack(2, required,
	  with_default(Record#ping.sessionid, none), string, [])];
iolist(pong, Record) ->
    [pack(1, required, with_default(Record#pong.id, none),
	  int32, []),
     pack(2, required,
	  with_default(Record#pong.sessionid, none), string, [])].
...
decode_pong(Bytes) when is_binary(Bytes) ->
    decode(pong, Bytes).

decode_ping(Bytes) when is_binary(Bytes) ->
    decode(ping, Bytes).

delimited_decode_ping(Bytes) ->
    delimited_decode(ping, Bytes).

delimited_decode_pong(Bytes) ->
    delimited_decode(pong, Bytes).
...
decode(ping, Bytes) when is_binary(Bytes) ->
    Types = [{2, sessionid, string, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(ping, Decoded);
decode(pong, Bytes) when is_binary(Bytes) ->
    Types = [{2, sessionid, string, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pong, Decoded).
...
to_record(ping, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, ping),
						   Record, Name, Val)
			  end,
			  #ping{}, DecodedTuples),
    Record1;
to_record(pong, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pong),
						   Record, Name, Val)
			  end,
			  #pong{}, DecodedTuples),
    Record1.
